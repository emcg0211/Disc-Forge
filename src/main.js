...
ipcMain.handle('disc:burn', async (_, { isoPath, deviceNode, verify = false, erase = false } = {}) => {
  if (burnInProgress) {
    return { success: false, error: 'A burn is already in progress. Please wait for it to complete.' };
  }
  burnInProgress = true;
  try {
  const { burnWithMediaCheck, verifyBurn } = require('./lib/burn');
  sendLog(`[burn] starting: ${isoPath} → ${deviceNode || '(no device)'}${verify ? ' (verify after burn)' : ''}${erase ? ' (erase first)' : ''}`);
  ...
  // progress lines, so the bar tracks the actual write position. Stages flow
  // 'checking' → ('erasing' →) 'burning' (see burnWithMediaCheck — the
  // erase-and-burn path for used BD-RE media, with node re-resolution).
  let lastBurnLine = '';
  let currentStage = 'burning';
  const STAGE_MSG = { checking: 'Checking the inserted disc…', erasing: 'Erasing disc — this takes about a minute…', burning: 'Writing to disc…' };
  const result = await burnWithMediaCheck({
    isoPath,
    deviceNode,
    erase,
    ...
    onStage: (stage) => {
      currentStage = stage;
      sendLog(`[burn] stage: ${stage}`);
      try { mainWindow?.webContents.send('burn-progress', { status: stage, message: STAGE_MSG[stage] || stage, percent: null }); } catch (_) {}
    },
    onLog: (line) => {
      lastBurnLine = line;
      sendLog('[burn] ' + line);
      try { mainWindow?.webContents.send('burn-progress', { status: currentStage, message: line, percent: null }); } catch (_) {}
    },
    onProgress: (percent) => {
      try { mainWindow?.webContents.send('burn-progress', { status: 'burning', message: lastBurnLine, percent }); } catch (_) {}
    },
  });
  // A needs-erase response is a question for the user, not an outcome: no
  // notification, no done/error event — the renderer shows the confirm and
  // re-invokes with erase:true.
  if (result.needsErase) {
    sendLog('[burn] media is non-blank rewritable — asking the user about erasing');
    return result;
  }
  ...
  if (result.success && verify) {
    ...
    // result.deviceNode is the node actually burned to — after an erase the
    // pre-erase node is dead (the burner re-enumerates).
    const v = await verifyBurn({ isoPath, deviceNode: result.deviceNode || deviceNode });
...
async function addSplashToDisc(bdFolder, splashPngPath, workDir, duration = 5) {
  ...
  try {
    ...
    // Step 2: mkvmerge TS → MKV
    await new Promise((resolve, reject) => {
      const mkv = spawn(TOOLS.mkvmerge, ['-o', splashMkv, splashTs]);
      mkv.on('error', reject);
      mkv.on('close', code => {
        if ((code !== 0 && code !== 1) || !fs.existsSync(splashMkv)) {
          reject(new Error(`mkvmerge exit ${code}`));
        } else {
          sendLog(`[Splash] Splash MKV: ${(fs.statSync(splashMkv).size/1e3).toFixed(0)} KB`);
          resolve();
        }
      });
    });
    ...
    if (fs.existsSync(splashMpls)) {
      ...
      if (fs.existsSync(backDir)) {
        ...
        fs.mkdirSync(path.dirname(backMpls), { recursive: true });
    ...
    // Step 5: Guard + patch MovieObject.bdmv Object[2]
    // Insert PLAY_PL(0) before the last command of Object[2].
    // Guard: last command must be PLAY_PL (opcode 0x22800000) — if not, disc layout is
    // unexpected and we must not blindly modify it.
    ...
    const lastCmdOpcode = obj2Body.readUInt32BE((origNumCmds - 1) * 12);
    if (lastCmdOpcode !== 0x22800000) {
      sendLog(`[Splash] Object[2] last command opcode 0x${lastCmdOpcode.toString(16).padStart(8,'0')} is not PLAY_PL (0x22800000) — skipping navigation patch`);
      return;
    }

    const PLAY_PL_SPLASH = Buffer.alloc(12);
    PLAY_PL_SPLASH.writeUInt32BE(0x22800000, 0);
    PLAY_PL_SPLASH.writeUInt32BE(0x00000000, 4);
    PLAY_PL_SPLASH.writeUInt32BE(0x00000000, 8);

    const newNumCmds = origNumCmds + 1;
    const newObj2    = Buffer.alloc(4 + newNumCmds * 12);
    newObj2.writeUInt16BE(mobj.readUInt16BE(obj2Start), 0);
    newObj2.writeUInt16BE(newNumCmds, 2);
    obj2Body.slice(0, (origNumCmds - 1) * 12).copy(newObj2, 4);
    PLAY_PL_SPLASH.copy(newObj2, 4 + (origNumCmds - 1) * 12);
    obj2Body.slice((origNumCmds - 1) * 12).copy(newObj2, 4 + origNumCmds * 12);

    const newMobj = Buffer.concat([mobj.slice(0, obj2Start), newObj2, mobj.slice(obj2Start + obj2Size)]);
    newMobj.writeUInt32BE(mobjLength + 12, MOBJ_STRUCT_OFF);
    ...
    sendLog(`[Splash] Error: ${e.message} — splash skipped`);
  } finally {
    if (fs.existsSync(splashBd)) {
      fs.rmSync(splashBd, { recursive: true, force: true });
    }
  }
}
...
function fixMultiTitleNavigationForEpisodes(bdFolder, numEpisodes, ep1TsMuxerPrefix, ep1BdTarget) {
  ...
  // FirstPlay → obj[2] (bypass obj[0]/obj[1] which reference stale playlist 0)
  buildHdmvEntry(2, 1).copy(newIndexes, 4);
  // TopMenu → obj[2] (PlayPL(1))
  buildHdmvEntry(2, 1).copy(newIndexes, 4 + ENTRY_SIZE);
  // num_titles
