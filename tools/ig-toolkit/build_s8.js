...
function main() {
  ...
  // 4. re-time: align earliest IG DTS to firstVideoPTS
  const earliest = Math.min(...manifest.units.map(u => (u.pes.dts != null ? u.pes.dts : u.pes.pts)));
  const offset = firstVideoPTS - earliest;
  console.log(`S8: IG earliest DTS=${earliest}, offset=${offset} → ICS PTS land at firstVideoPTS+12012 …`);
  ...
  // 8. ISO
  const iso = path.join(DESKTOP, 'toast_S8.iso');
